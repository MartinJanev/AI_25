...
class Puzzle(Problem):
    ...
    def successor(self, state):
        ...
        successors = dict()
        ind = state.index("*")  # index of blank field in state

        # Up - means we move the index of blank field up by 3 (-3)
        if ind > 2:  # if it is in the first row, we can't move it up
            tmp = list(state)
            tmp[ind], tmp[ind - 3] = tmp[ind - 3], tmp[ind]
            new_state = ''.join(tmp)
            successors["Up"] = new_state

        # Down - means we move the index of blank field down by 3 (+3)
        if ind < 6:  # if it is in the last row, we can't move it down
            tmp = list(state)
            tmp[ind], tmp[ind + 3] = tmp[ind + 3], tmp[ind]
            new_state = ''.join(tmp)
            successors["Down"] = new_state

        # Left - means we move the index of blank field left by 1 (-1)
        if ind % 3 != 0:  # if it is in the first column, we can't move it left
            tmp = list(state)
            tmp[ind], tmp[ind - 1] = tmp[ind - 1], tmp[ind]
            new_state = ''.join(tmp)
            successors["Left"] = new_state

        # Right - means we move the index of blank field right by 1 (+1)
        if ind % 3 != 2:  # if it is in the last column, we can't move it right
            tmp = list(state)
            tmp[ind], tmp[ind + 1] = tmp[ind + 1], tmp[ind]
            new_state = ''.join(tmp)
