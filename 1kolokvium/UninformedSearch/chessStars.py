...
class Stars(Problem):
    ...
    def successor(self, state):
        ...
        for i, move_func in enumerate([k1, k2, k3, k4, k5, k6, k7, k8], start=1):
            new_x, new_y = move_func(knight_x, knight_y, bishop_x, bishop_y)
            if [knight_x, knight_y] != [new_x, new_y]:
                successors[f'K{i}'] = (new_x, new_y, bishop_x, bishop_y,
                                       tuple([s for s in stars_positions if s[0] != new_x or s[1] != new_y]))

        for i, move_func in enumerate([b1, b2, b3, b4], start=1):
            new_x, new_y = move_func(bishop_x, bishop_y, knight_x, knight_y)
            if [bishop_x, bishop_y] != [new_x, new_y]:
                successors[f'B{i}'] = (knight_x, knight_y, new_x, new_y,
                                       tuple([s for s in stars_positions if s[0] != new_x or s[1] != new_y]))
                return successors
