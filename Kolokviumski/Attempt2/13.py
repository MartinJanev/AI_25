...
def modelGenerator(st):
    if st == "NB":
        return GaussianNB()
    if st == "DT":
        return DecisionTreeClassifier(random_state=0)
    if st == "MLP":
        return RandomForestClassifier(n_estimators=3, random_state=0)


def f(data, index):
    return int(index * len(data))
...
def acc_evaluate(bayes: GaussianNB, tree: DecisionTreeClassifier, forest: RandomForestClassifier, \
                 test_X, test_y):
    acc = 0
    for x, y in zip(test_X, test_y):
        brojac = 0
        p0 = bayes.predict([x])[0]
        p1 = tree.predict([x])[0]
        p2 = forest.predict([x])[0]
...
if __name__ == '__main__':
    ...
    bayes = modelGenerator("NB")
    tree = modelGenerator("DT")
    forest = modelGenerator("MLP")

    models = [bayes, tree, forest]

    c0 = [row for row in dataset if row[-1] == 0]
    c1 = [row for row in dataset if row[-1] == 1]
    c2 = [row for row in dataset if row[-1] == 2]

    test_set = c0[f(c0, x2):] + c1[f(c1, x2):] + c2[f(c2, x2):]

    train_set_B = c0[:f(c0, x1)] + c1[:f(c1, x1)] + c2[:f(c2, x1)]
    train_set_T = c0[f(c0, x1):f(c0, x2)] + c1[f(c1, x1):f(c1, x2)] + c2[f(c2, x1):f(c2, x2)]
    train_set_F = c0[:f(c0, x2)] + c1[:f(c1, x2)] + c2[:f(c2, x2)]

    test_X, test_y = split_(test_set)

    X0, Y0 = split_(train_set_B)
    X1, Y1 = split_(train_set_T)
    X2, Y2 = split_(train_set_F)

    bayes.fit(X0, Y0)
    tree.fit(X1, Y1)
    forest.fit(X2, Y2)

    print(f"Tochnost: {acc_evaluate(bayes, tree, forest, test_X, test_y)}")
