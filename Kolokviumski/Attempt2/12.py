...
data = [[30, 92726, 2.0, 9, 2011, -37.8497, 144.968, 10, 0],
...
def accuracy(prediction, Y):
    return sum(1 for true, pred in zip(Y, prediction) if true == pred) / len(Y)
...
if __name__ == '__main__':
    ...
    layers = int(input())
    rate = float(input())
    col_remove = int(input())
    example = [float(x) for x in input().strip().split(" ")]
    ...
    acc1 = accuracy(pred_tr, train_y)
    acc2 = accuracy(pred_val, val_y)

    if acc1 >= 1.15 * acc2:
        new_train_X = [row[:col_remove] + row[col_remove + 1:] for row in train_X]
        scaler.fit(new_train_X)
        new_train_X = scaler.transform(new_train_X)
        model.fit(new_train_X, train_y)
        example = [float(i) for i in range(len(example)) if i != col_remove]
        print("Se sluchuva overfitting")
    else:
        print("Ne se sluchuva overfitting")

    print(model.predict(scaler.transform([example]))[0])
