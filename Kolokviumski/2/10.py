...
if __name__ == '__main__':
    ...
    y = [1 if row[-1] >= 5 else 0 for row in dataset]
    ...
    min_importance_index = np.argmin(tree.feature_importances_)

    X_train_red = remove_column(X_train, min_importance_index)
    X_test_red = remove_column(X_test, min_importance_index)

    scaler1 = StandardScaler()
    X_train_1 = scaler1.fit_transform(X_train_red)
    X_test_1 = scaler1.transform(X_test_red)
    ...
    prediction1 = mlp.predict(X_test_1)
    acc_std = sum(1 for pred, true in zip(prediction1, y_test) if pred == true) / len(y_test)

    # Тренирање на MLP со MinMaxScaler податоци
    scaler_minmax = MinMaxScaler()
    X_train_minmax = scaler_minmax.fit_transform(X_train_red)
    X_test_minmax = scaler_minmax.transform(X_test_red)

    mlp.fit(X_train_minmax, y_train)
    prediction2 = mlp.predict(X_test_minmax)
    acc_minmax = sum(1 for pred, true in zip(prediction2, y_test) if pred == true) / len(y_test)
