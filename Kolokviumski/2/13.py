...
def f(set_data, percent):
    return int(percent * len(set_data))


def generateClassificator(cf):
    if cf == "NB":
        return GaussianNB()
    if cf == "DT":
...
def acc_evaluate(c0: GaussianNB, c1: DecisionTreeClassifier, c2: RandomForestClassifier, test_X, test_Y):
    acc = 0
...
if __name__ == '__main__':
    ...
    NB = generateClassificator("NB")
    DT = generateClassificator("DT")
    RF = generateClassificator("RFC")
    ...
    # from x2 to the end
    test_set = (set_0[f(set_0, x2):]
                + set_1[f(set_1, x2):]
                + set_2[f(set_2, x2):])
    # first x1 percent
    train_set1 = (set_0[:f(set_0, x1)]
                  + set_1[:f(set_1, x1)]
                  + set_2[:f(set_2, x1)])
    # from x1 to x2
    train_set2 = (
            set_0[f(set_0, x1):f(set_0, x2)]
            + set_1[f(set_1, x1):f(set_1, x2)]
            + set_2[f(set_2, x1):f(set_2, x2)])
    # first x2 percent
    train_set3 = (set_0[:f(set_0, x2)]
                  + set_1[:f(set_1, x2)]
                  + set_2[:f(set_2, x2)])

    test_X, test_Y = split_data(test_set)
    X0, Y0 = split_data(train_set1)
    NB.fit(X0, Y0)
    X1, Y1 = split_data(train_set2)
    DT.fit(X1, Y1)
    X2, Y2 = split_data(train_set3)
    RF.fit(X2, Y2)

    print(f"Tochnost: {acc_evaluate(NB, DT, RF, test_X, test_Y)}")
