...
if __name__ == '__main__':
    ...
    arr = [new_input[i] for i in range(len(new_input)) if i != col]

    temp_dataset = list()

    for row in dataset:
        temp_dataset.append([row[i] for i in range(len(row)) if i != col])
    dataset_updated = temp_dataset
    ...
    # max_importance = list(classifier.feature_importances_).index(max(classifier.feature_importances_))
    predictions = classifier.predict(test_X)

    acc = sum(1 for true, pred in zip(test_Y, predictions) if true == pred) / len(test_Y)
    print(f"Accuracy: {acc}")
    print(classifier.predict([arr])[0])
    print(classifier.predict_proba([arr])[0])
