...
if __name__ == '__main__':
    ...
    x_transform = scaler.transform(train_x)
    val_x = scaler.transform(val_x)

    classifer = MLPClassifier(random_state=0,
                              activation='relu',
                              max_iter=20,
                              learning_rate_init=rate_learn,
                              hidden_layer_sizes=n_neurons)

    classifer.fit(x_transform, train_y)
    prediction_train = classifer.predict(x_transform)
    prediction_val = classifer.predict(val_x)

    acc1 = accuracy(prediction_train, train_y)
    acc2 = accuracy(prediction_val, val_y)

    if acc1 >= acc2 * 1.15:
        new_train_x = [row[:col_delete] + row[col_delete + 1:] for row in train_x]
        scaler.fit(new_train_x)
        x_transform = scaler.transform(new_train_x)
        classifer.fit(x_transform, train_y)
        inp = [float(x) for x in inp[:col_delete] + inp[col_delete + 1:]]
        print("Se sluchuva overfitting")
    else:
        print("Ne se sluchuva overfitting")

    input_transform = scaler.transform([inp])
    print(classifer.predict(input_transform)[0])
