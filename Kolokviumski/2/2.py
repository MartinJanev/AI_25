...
if __name__ == '__main__':
    ...
    threshold = int((85 * len(dataset)) / 100)
    train_X = X[:threshold]
    test_X = X[threshold:]
    train_Y = Y[:threshold]
    test_Y = Y[threshold:]
    ...
    input_block = input().strip().split(" ")
    input_block = list(map(float, input_block))
    input_encoded = [input_block]

    pred_class = classifier.predict(input_encoded)[0]

    print(accuracy)
    print(pred_class)
    print(classifier.predict_proba(input_encoded))
