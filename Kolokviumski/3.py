        ...
        for i, pillar_1 in enumerate(state):
            ...
            for j, pillar_2 in enumerate(state):
                ...
                if len(pillar_2) == 0 or top_block <= pillar_2[-1]:
                    successors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = self.create_state(state, i,
                                                                                                            j)

        return successors

    def create_state(self, state, i, j):
...
if __name__ == '__main__':
    ...
    problem = Pillar(initial_state, goal_state)
