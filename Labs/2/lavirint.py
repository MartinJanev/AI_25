...
class MazeSolver(Problem):
    ...
    def is_valid_move(self, action, x, y):
        if action.startswith("Desno"):
            step = 1 if action == "Desno 2" else 2  # Move 2 or 3 spaces
            intermediate_positions = [(x + i, y) for i in range(1, step + 1)]
            if any(pos in self.walls for pos in intermediate_positions):  # Check walls along path
                return False  # Skip this move
        return True
    ...
    def successor(self, state):
        ...
        for action, move in MOVEMENTS.items():
            ...
            if self.is_valid_move(action, x, y):
                if self.is_valid((new_x, new_y)):  # Final position must be valid
...
# super().__init__((initial, None), goal) #none-prev_move
# (x, y), prev_move = state
# if prev_move and prev_move.split()[0] == action.split()[0]:
#     continue
# if self.is_valid_move(action, x, y) and self.is_valid((new_x, new_y)):
