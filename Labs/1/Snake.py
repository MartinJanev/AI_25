...
def valid_snake(snake, redApples):
    ...
    if 0 <= snake[-1][0] < WIDTH and 0 <= snake[-1][1] < HEIGHT:
        return True
    return False
...
class Snake(Problem):
    ...
    def successor(self, state):
        successors = {}
        forward_state = move_forward(state)
        right_state = move_right(state)
        left_state = move_left(state)

        if valid_snake(forward_state[0], self.redApples):
            successors["ProdolzhiPravo"] = forward_state
        if valid_snake(right_state[0], self.redApples):
            successors["SvrtiDesno"] = right_state
        if valid_snake(left_state[0], self.redApples):
...
if __name__ == '__main__':
    ...
    greenApples = []
    for i in range(n):
        greenApples.append(tuple(int(x) for x in input().split(",")))

    m = int(input())
    redApples = []
    for i in range(m):
        redApples.append(tuple(int(x) for x in input().split(",")))
