...
def aroundOpp(opponents):
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            surrounding_positions.add((opponents[0] + i, opponents[1] + j))
...
def check_state(state, opponents):
    # Check if player is on the same position as the opponents
    if state[0] in opponents:
        return False
    # Check if player is on the same position as the ball
    if state[1] in aroundOpp(opponents[0]) or state[1] in aroundOpp(opponents[1]):
        return False
    # Check if player is on the same position as the goal block
...
class Football(Problem):
    ...
    def successor(self, state):
        ...
        for move, actionBall, actionNoBall in zip(ways_to_move, actions_with_ball, actions_no_ball):
            new_coords = (player[0] + move[0], player[1] + move[1])
            if new_coords == ball:
                new_ball = (new_coords[0] + move[0], new_coords[1] + move[1])
